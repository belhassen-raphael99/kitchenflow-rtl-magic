...
import { 
  ...
  ChefHat, Clock, TrendingUp, Warehouse, BookOpen,
  ...
  return (
    <div className="space-y-4 sm:space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
        ...
        <Card className={`border-r-4 ${criticalStockItems > 0 ? 'border-r-destructive' : lowStockItems > 0 ? 'border-r-amber-500' : 'border-r-primary'}`}>
          <CardHeader className="flex flex-row items-center justify-between pb-1 sm:pb-2 p-3 sm:p-6">
            <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground">התראות מלאי</CardTitle>
            <AlertTriangle className={`w-4 h-4 sm:w-5 sm:h-5 ${criticalStockItems > 0 ? 'text-destructive' : lowStockItems > 0 ? 'text-amber-500' : 'text-primary'}`} />
          </CardHeader>
          <CardContent className="p-3 sm:p-6 pt-0">
            {statsLoading ? <SkeletonValue /> : (
              ...
              <div className="flex flex-wrap gap-1 sm:gap-2 mt-1">
                ...
                {criticalStockItems === 0 && lowStockItems === 0 && <Badge className="bg-primary text-[10px] sm:text-xs px-1.5 sm:px-2">מלאי תקין</Badge>}
              </div></>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-r-4 border-r-primary">
          ...
          <CardContent className="p-3 sm:p-6 pt-0">
            {statsLoading ? <SkeletonValue /> : (
              ...
              <p className="text-[10px] sm:text-xs text-muted-foreground">{guestsThisWeek} אורחים</p></>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-r-4 border-r-amber-500">
          <CardHeader className="flex flex-row items-center justify-between pb-1 sm:pb-2 p-3 sm:p-6">
            <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground">ממתינים לתשלום</CardTitle>
            <Receipt className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500" />
          </CardHeader>
          <CardContent className="p-3 sm:p-6 pt-0">
            {statsLoading ? <SkeletonValue /> : (
              <><p className="text-2xl sm:text-3xl font-bold">{pendingInvoices}</p>
              <p className="text-[10px] sm:text-xs text-muted-foreground">חשבוניות פתוחות</p></>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            {statsLoading ? (
              ...
              <div className="bg-muted/50 rounded-lg p-4 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={nextEvent.daysUntil <= 2 ? 'destructive' : 'secondary'}>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 sm:space-y-4 p-4 sm:p-6 pt-0">
            {[
              { icon: Warehouse, label: 'מחסן', sub: `${totalWarehouseItems} פריטים`, link: '/warehouse' },
              { icon: BookOpen, label: 'ספר מתכונים', sub: `${totalRecipes} מתכונים`, link: '/recipes' },
              { icon: CalendarDays, label: 'יומן אירועים', sub: `${eventsThisWeek} אירועים השבוע`, link: '/agenda' },
              { icon: ChefHat, label: 'פוסט מטבח', sub: `${activeTasks} משימות פעילות`, link: '/kitchen-ops' },
            ].map(item => (
              <Link key={item.label} to={item.link} className="flex items-center justify-between p-2.5 sm:p-4 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
                ...
                <Badge className="bg-primary text-[10px] sm:text-xs">פעיל</Badge>
              </Link>
            ))}
          </CardContent>
        </Card>
